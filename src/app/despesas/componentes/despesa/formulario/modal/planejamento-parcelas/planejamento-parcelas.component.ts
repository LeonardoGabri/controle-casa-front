...
export class PlanejamentoParcelasComponent implements OnInit{
  ...
  configurarMenuAcoes() {
    this.menuAcoes = [
      { label: 'Editar', icon: 'pi pi-pencil', command: () => this.editarParcela.bind(this) },
      { label: 'Remover', icon: 'pi pi-trash', command: () => this.removerParcela.bind(this) }
    ];
  }
  ...
  atualizaNomeResponsavel(item: any){
    this.formulario.get('responsavelNome')?.setValue(item.originalEvent.currentTarget.ariaLabel);
  }
